...
#define DI_BASE_END (DI_BASE_OFFSET+DI_END) // 13
...
//ModbusSlaveRTU rtu(&mem, &softSerial);
ModbusSlaveRTU rtu(&mem, &Serial1);
...
void setup()
{
   // Open serial communications and wait for port to open:
    ...
    while (!Serial) // wait for serial port to connect. Needed for native USB port only
        delay(1);
    Serial.println("============================================================");
    Serial.println("================= MODBUS SLAVE RTU EXAMPLE =================");
    Serial.println("============================================================");
    ...
    Serial.println("Initialize SofwareSerial instance and set parameters as 9600 speed");
    //pinMode(rxPin, INPUT);
...
Modbus::Response r = Modbus::PROCESSING;
...
void loop()
{
    ...
    // 400002 - error counter
    // 400003 - last error
    mem.setUInt16_4x(0, mem.uint16_4x(0)+(r==Modbus::OK)); // success counter
    mem.setUInt16_4x(1, mem.uint16_4x(1)+(r>Modbus::OK)); // error counter
    if (r > Modbus::OK)
        mem.setInt_4x(2, r); // last error
    // ------------------------------------------------
    // 3. MODBUS EXCHANGE
    r = rtu.exec();
    // 4. WRITE DATA
